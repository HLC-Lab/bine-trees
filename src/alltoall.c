...
/**
 * \brief Butterfly-based implementation of the alltoall collective operation.
 * This implementation is designed for small messages, similarly to Bruck's algorithm.
 */
int bine_alltoall_small(const void *sendbuf, size_t s_count, MPI_Datatype s_dtype,
                   void *recvbuf, size_t r_count, MPI_Datatype r_dtype, MPI_Comm comm)
{
  ...
  char *tmpbuf = NULL;
  uint32_t *resident_block, *resident_block_next;
  ...
  tmpbuf_size_real = tmpbuf_size + sizeof(uint32_t) * size + sizeof(uint32_t) * size;
  ...
  resident_block = (uint32_t *) (tmpbuf + tmpbuf_size);
  resident_block_next = (uint32_t *) (tmpbuf + tmpbuf_size + sizeof(uint32_t) * size);
  ...
  while(mask < size){
    ...
    min_block_s = nu(partner, size) & block_first_mask;
    max_block_s = min_block_s + inverse_mask - 1;

    int block_recvd_cnt = 0, block_send_cnt = 0;
    size_t offset_send = 0, offset_keep = 0;
    num_resident_blocks_next = 0;
    for(int i = 0; i < size; i++){
      uint32_t block = resident_block[i % num_resident_blocks];
      // Shall I send this block? Check the negabinary thing  
      uint32_t remap_block = nu(block, size);
    ...
    memcpy(resident_block, resident_block_next, sizeof(uint32_t) * num_resident_blocks);
